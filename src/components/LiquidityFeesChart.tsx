          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-gray-50 p-4 rounded-md">
              ...
              <div className="text-sm">
                <p>
                  <span className="font-medium">トークンペア:</span>{" "}
                  {token0Symbol} / {token1Symbol}
                </p>
                <p>
                  <span className="font-medium">総流動性:</span>{" "}
                  {formatUSD(data[data.length - 1]?.tvlUSD)}
                </p>
                <p>
                  <span className="font-medium">累積手数料:</span>{" "}
                  {formatUSD(data[data.length - 1]?.feesUSD)}
                </p>
                {feeTier && (
                  <p>
                    <span className="font-medium">手数料率:</span>{" "}
                    {Number(feeTier) / 10000}%
                  </p>
                )}
              </div>
            </div>
                ...
                {data.length > 1 && (
                  <>
                    <p>
                      ...
                      {formatUSD(
                        data[data.length - 1]?.tvlUSD - data[0]?.tvlUSD,
                      )}{" "}
                      (
                      {(
                        (data[data.length - 1]?.tvlUSD / data[0]?.tvlUSD - 1) *
                        100
                      ).toFixed(2)}
                      %)
                    </p>
                    <p>
                      ...
                      {formatUSD(
                        data[data.length - 1]?.feesUSD - data[0]?.feesUSD,
                      )}
                    </p>
                  </>
                )}
