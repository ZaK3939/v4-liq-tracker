...
export function useLiquidityHistory(
  ...
  return useMemo(() => {
    ...
    // 初期値を設定
    const initialLiquidity = poolDetails.liquidity
      ? BigInt(poolDetails.liquidity)
      : BigInt(0);
    ...
    const chartData: ChartDataPoint[] = timestamps.map((timestamp) => {
      ...
      const liquidityEventsUntilTimestamp = sortedLiquidityEvents.filter(
        (e) => parseInt(e.timestamp) <= timestamp,
      );

      const swapEventsUntilTimestamp = sortedSwapEvents.filter(
        (e) => parseInt(e.timestamp) <= timestamp,
      );

      // 流動性と価値の計算
      let runningLiquidity = Number(initialLiquidity);
      let runningTVL = initialTVL;
      ...
      const lastSwap =
        swapEventsUntilTimestamp[swapEventsUntilTimestamp.length - 1];
      const sqrtPrice = lastSwap
        ? lastSwap.sqrtPriceX96
        : poolDetails.sqrtPrice;
      const tick = lastSwap
        ? parseInt(lastSwap.tick || poolDetails.tick)
        : parseInt(poolDetails.tick);
